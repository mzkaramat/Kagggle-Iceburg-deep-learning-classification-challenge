...
class AutoEncoder(nn.Module):
    ...
    def __init__(self, code_size):
        ...
        self.enc_cnn_1 = nn.Conv2d(2, 10, kernel_size=4)
        self.enc_cnn_2 = nn.Conv2d(10, 15, kernel_size=5)
        self.enc_cnn_3 = nn.Conv2d(15, 20, kernel_size=5)
        self.enc_cnn_4 = nn.Conv2d(20, 25, kernel_size=5)
        self.enc_linear_1 = nn.Linear(4225, 2000)
        self.enc_linear_2 = nn.Linear(2000, 1500)
        self.enc_linear_3 = nn.Linear(1500, self.code_size)
        
        # Decoder specification
        self.dec_linear_1 = nn.Linear(self.code_size, 1500)
        self.dec_linear_2 = nn.Linear(1500, 2000)
        self.dec_linear_3 = nn.Linear(2000,4225)
        self.dec_cnn_1 =  nn.ConvTranspose2d(25, 20, kernel_size=5)
        self.dec_cnn_2 = nn.ConvTranspose2d(20, 15, kernel_size=5)
        self.dec_cnn_3 =  nn.ConvTranspose2d(15, 10, kernel_size=5)
        self.dec_cnn_4 = nn.ConvTranspose2d(10, 2, kernel_size=4)
    def forward(self, images):
        code,indices1,indices2 = self.encode(images)
        out = self.decode(code,indices1,indices2)
        return out, code
    ...
    def encode(self, images):
        ...
        #print(code.shape)
        code,indices1 = F.max_pool2d(code, 2, return_indices=True)
        #print(code.shape)
        code = F.relu(self.enc_cnn_3(code))
        #print(code.shape)
        code = F.relu(self.enc_cnn_4(code))
        #print(code.shape)
        code,indices2 = F.max_pool2d(code, 2, return_indices=True)
        #print(code.shape)
        ...
        #print(code.shape)
        code = F.relu(self.enc_linear_3(code))
        #print(code.shape)
        return code,indices1,indices2
    
    def decode(self, code,indices1,indices2):
        #print(code.shape)
        out = F.relu(self.dec_linear_1(code))
        #print(out.shape)
        out = F.relu(self.dec_linear_2(out))
        #print(out.shape)
        out = F.relu(self.dec_linear_3(out))
        #print(out.shape)
        out = out.view([out.size(0), 25, 13, 13])
        #print(out.shape)
        ##print(indices2.shape)
        out = F.max_unpool2d(out, indices2,kernel_size=2,stride=2)
        #print(out.shape)
        ...
        #print(out.shape)
        out =  F.max_unpool2d(out,indices1 ,kernel_size=2,stride=2)
        #print(out.shape)
...
for i in range(len(rates)):
    lr = rates[i]
    optimizer = optimizer_cls(model.parameters(), lr=lr,weight_decay=0.015)
    all_losses_training = []
...
torch.save(model.state_dict(), './conv_autoencoder_final.pth')
...
conv_features.shape
np.savetxt("lat_features_final.csv", conv_features, delimiter=",")
